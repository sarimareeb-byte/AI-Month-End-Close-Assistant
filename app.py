...
from prepaid_data_adapter import process_raw_files, mapping_to_table
...
def build_excel_package(results, risk_label, focus_text):
    ...
    agg     = results.get("aggregate_reconciliation_summary")

    # Use aggregate reconciliation for headline metrics when available
    if agg is not None and not agg.empty:
        n_total  = len(agg)
        n_recon  = int((agg["status"] == "RECONCILED").sum())
        n_exc    = int((agg["status"] == "EXCEPTION").sum())
        rate     = (n_recon / n_total * 100) if n_total > 0 else 0.0
        t_var    = agg["abs_variance"].sum()
        _rlabel  = "Entities"
    else:
        n_total  = len(recon)
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        ...
        exec_data = [
            ...
            [f"Reconciled {_rlabel}", n_recon],
            [f"Exception {_rlabel}", n_exc],
            [f"{_rlabel} Reconciliation Rate", f"{rate:.1f}%"],
        ...
        # ── Sheet 3: Aggregate Reconciliation (primary) ────────
        if agg is not None and not agg.empty:
            agg_cols = [c for c in ["entity", "expected_balance", "actual_balance",
                        "variance", "abs_variance", "variance_percent", "status"]
                        if c in agg.columns]
            agg[agg_cols].to_excel(writer, sheet_name="Aggregate Recon", index=False)
            _style_sheet(writer.sheets["Aggregate Recon"], "Aggregate Recon",
                         dollar_cols=["expected_balance", "actual_balance",
                                      "variance", "abs_variance"])

        # ── Sheet 4: Period-Level Reconciliation (drill-down) ──
...
with st.sidebar:
    ...
    input_mode = st.radio(
        "Input mode",
        ["Standard CSV", "Raw Prepaid Export"],
        index=0,
        help="Standard: pre-formatted CSVs matching the required schema. "
             "Raw: Excel/CSV exports from any ERP — the adapter normalizes them.",
    )

    if input_mode == "Standard CSV":
        st.caption("Both files must follow the required schema.")
        ledger_file = st.file_uploader("Prepaid Expense Ledger (CSV)",
                                        type=["csv"], key="std_l")
        amort_file  = st.file_uploader("Amortization Schedule (CSV)",
                                        type=["csv"], key="std_a")
        both = ledger_file is not None and amort_file is not None
    else:
        st.caption("Upload raw ERP exports — columns are auto-mapped.")
        ledger_file = st.file_uploader(
            "Ledger / GL Detail Export",
            type=["csv", "xlsx", "xls"], key="raw_l",
            help="Transaction-level prepaid GL export.",
        )
        amort_file = st.file_uploader(
            "Amortization Schedule Export",
            type=["csv", "xlsx", "xls"], key="raw_a",
            help="Amortization schedule list export.",
        )
        entity_name = st.text_input(
            "Entity name",
            placeholder="e.g. accessiBe Inc",
            help="Added to every row if the export has no entity column.",
        )
        both = ledger_file is not None and amort_file is not None
    ...
    if not both:
        st.info("Upload both files to enable analysis.", icon="📁")
    st.divider()
    ...
    try:
        with st.spinner("Running reconciliation analysis..."):
            ...
            if input_mode == "Standard CSV":
                ...
                # ── Raw export: adapter pipeline ──────────
                ent = entity_name.strip() if entity_name else None
                adapter_out = process_raw_files(ledger_file, amort_file, entity=ent)
                st.session_state.adapter_out = adapter_out
                report = adapter_out["validation_report"]

                if not adapter_out["can_run"]:
                    raise ValueError(report["summary"])

                st.session_state.results = run_analysis(
                    adapter_out["ledger_df"], adapter_out["amort_df"])

        st.success("Analysis complete.", icon="✅")

        # Show adapter feedback if raw mode
        if input_mode != "Standard CSV" and "adapter_out" in st.session_state:
            ao = st.session_state.adapter_out
            rpt = ao["validation_report"]

            # Validation status
            all_warns = ao["ledger_warnings"] + ao["amort_warnings"]
            dq = rpt.get("data_quality_warnings", [])
            lk = rpt.get("linkage_warnings", [])
            total_w = len(all_warns) + len(dq) + len(lk)

            if total_w:
                with st.expander(f"⚠️ Adapter warnings ({total_w})", expanded=False):
                    for w in all_warns:
                        st.write(f"• {w}")
                    for w in dq:
                        st.write(f"• {w}")
            ...
            with st.expander("📋 Column mapping report", expanded=False):
                st.write("**Ledger mapping:**")
                st.dataframe(pd.DataFrame(mapping_to_table(ao["ledger_mapping"])),
                             use_container_width=True, hide_index=True)
                st.write("**Amortization mapping:**")
                st.dataframe(pd.DataFrame(mapping_to_table(ao["amort_mapping"])),
                             use_container_width=True, hide_index=True)
        ...
        if input_mode != "Standard CSV" and "adapter_out" in st.session_state:
            rpt = st.session_state.adapter_out.get("validation_report", {})
            fixes = rpt.get("suggested_fixes", [])
            if fixes:
                for fix in fixes:
                    st.warning(fix, icon="💡")
...
# ── Headline metrics: aggregate reconciliation (primary) ──
# Period-level recon stays for drill-down tabs; aggregate drives KPIs.
agg_recon = results.get("aggregate_reconciliation_summary")
if agg_recon is not None and not agg_recon.empty:
    n_total      = len(agg_recon)
    n_reconciled = int((agg_recon["status"] == "RECONCILED").sum())
    n_exc        = int((agg_recon["status"] == "EXCEPTION").sum())
    rate         = (n_reconciled / n_total * 100) if n_total > 0 else 0.0
    total_var    = agg_recon["abs_variance"].sum()
    _recon_label = "Entity"
    _recon_plural = "Entities"
else:
    n_total      = len(recon)
    ...
    _recon_label = "Period"
    _recon_plural = "Periods"

# ═══════════════════════════════════════════════════════════
# 1 · CLOSE RISK BANNER
# ═══════════════════════════════════════════════════════════

agg_all_reconciled = (agg_recon is not None and not agg_recon.empty
                      and (agg_recon["status"] == "RECONCILED").all())

# Count material HIGH-confidence findings (not just any HIGH severity)
mat_threshold = info.get("materiality_threshold_used", 100.0)
n_material_high = 0
if not inv.empty:
    n_material_high = int(
        ((inv["severity"] == "HIGH")
         & (inv["estimated_impact"] >= mat_threshold)).sum()
    )

if agg_all_reconciled and n_material_high == 0:
    risk_level, risk_label, risk_cls = "LOW", "🟢 Low Close Risk", "rb-low"
elif agg_all_reconciled and n_material_high < 3:
    risk_level, risk_label, risk_cls = "MEDIUM", "🟡 Elevated Close Risk", "rb-med"
elif n_high >= 3 or rate < 50:
...
<div class="risk-banner {risk_cls}">
  ...
  <div class="rb-stats">
    ...
    <div class="rb-stat"><span class="rb-val">{n_exc} / {n_total}</span><span class="rb-lbl">Exception {_recon_plural}</span></div>
    <div class="rb-stat"><span class="rb-val">{rate:.0f}%</span><span class="rb-lbl">{_recon_label} Reconciliation</span></div>
  </div>
</div>
""", unsafe_allow_html=True)
...
c5.metric(f"Exception {_recon_plural}", f"{n_exc}/{n_total}")
c6.metric(f"{_recon_label} Recon Rate", f"{rate:.0f}%")
...
# ── Root cause summary (aggregated view) ──────────────────
rc_summary = results.get("root_cause_summary")
if rc_summary is not None and not rc_summary.empty:
    disp_cols = ["root_cause_category", "finding_count", "material_finding_count",
                 "total_impact", "material_impact", "highest_confidence"]
    available = [c for c in disp_cols if c in rc_summary.columns]
    summary_disp = rc_summary[available].copy()
    rename = {
    ...
    summary_disp = summary_disp.rename(columns=rename)
    if "Root Cause" in summary_disp.columns:
        summary_disp["Root Cause"] = (summary_disp["Root Cause"]
                                       .str.replace("_", " ").str.title())
    fmt = {}
    if "Total Impact" in summary_disp.columns:
        fmt["Total Impact"] = "${:,.2f}"
    if "Material Impact" in summary_disp.columns:
        fmt["Material Impact"] = "${:,.2f}"
    st.dataframe(summary_disp.style.format(fmt),
                 use_container_width=True, hide_index=True)

    # Show materiality threshold
    mat_val = info.get("materiality_threshold_used")
    if mat_val:
        st.caption(f"Materiality threshold: ${mat_val:,.2f}")
    st.write("")

if inv.empty:
    st.info("No corrective actions to display.", icon="ℹ️")
else:
    has_conf = "confidence" in inv.columns
    reg_cols = ["priority_rank", "issue_title", "entity", "severity"]
    reg_names = ["Priority", "Issue", "Entity", "Severity"]
    if has_conf:
        reg_cols.append("confidence")
        reg_names.append("Confidence")
    reg_cols += ["estimated_impact", "root_cause_id"]
    reg_names += ["Impact", "_rc_id"]

    register = inv[reg_cols].copy()
    register.columns = reg_names
