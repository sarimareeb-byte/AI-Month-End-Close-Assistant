...
def enrich_ledger_ids(ledger_df, amort_df):
    ...
    against source_document in the amortization schedule.
    ...
    # Build lookup: cleaned source_document → prepaid_item_id
    doc_to_pid = {}
    for _, row in amort_df.drop_duplicates("source_document").iterrows():
        sd = str(row.get("source_document", ""))
        clean = re.sub(r"^(Bill|Journal|Bill Credit)\s*#\s*", "", sd).strip()
        pid = str(row.get("prepaid_item_id", "")).strip()
        if clean and pid:
            doc_to_pid[clean] = pid

    matched = ledger_df["document_number"].astype(str).str.strip().map(doc_to_pid)
